...
app.get('/api/refresh', checkAuth, async (req, res) => {
    try {
        res.status(200).send({ message: 'Valid Token' }); // Send an error response if fetching data fails
    } catch (error) {
        // console.error('Error fetching data:', error); // Log any errors
        res.status(500).send({ message: 'InValid Token'}); // Send an error response if fetching data fails
    }
});
